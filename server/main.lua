...
ESX.RegisterServerCallback('esx_tattooshop:removeTattoo', function(source, cb, tattooList, price)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, _U('remove_tattoo', ESX.Math.GroupDigits(price)))
		cb(true)
	else
		local missingMoney = price - xPlayer.getMoney()
		TriggerClientEvent('esx:showNotification', source, _U('not_enough_money', ESX.Math.GroupDigits(missingMoney)))
