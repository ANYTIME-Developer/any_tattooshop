...
function OpenTattooMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop_menu', {
		title    = _U('tattoos'),
		align    = 'bottom-right',
		elements = {
			{label = 'Poner Tatuaje',			value = 'put_tatto'},
			{label = 'Quitar Tatuaje',			value = 'remove_tatto'},
		}
	}, function(data4, menu4)
		if data4.current.value == 'put_tatto' then
			OpenShopMenu()
		elseif data4.current.value == 'remove_tatto' then
			RemoveTattoMenu()
		end
...
function RemoveTattoMenu()
	local elements = {{label = _U('go_back_to_menu'), value = nil} }

	for i,k in pairs(currentTattoos) do
		table.insert(elements, {
			label = 'Tatuaje '..i..' - '.._U('money_amount', ESX.Math.GroupDigits(Config.PriceRemoveTattoo)),
			value = Config.TattooList[k.collection][k.texture],
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_removeMenu', {
		title    = _U('tattoos'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		if data.current.value ~= nil then
			...
			for i,k in pairs(currentTattoos) do
				if k.texture == data.current.texture and k.collection == data.current.collection then
					local nada
				else
					table.insert(newTattoos, {collection = k.collection, texture = k.texture})
				end
			end

			ESX.TriggerServerCallback('esx_tattooshop:removeTattoo', function(success)
				if success then
					currentTattoos = newTattoos
					cleanPlayer()
					menu.close()
				end
			end, newTattoos, Config.PriceRemoveTattoo)
		...
		if data.current.value ~= nil then
			drawTattoo(data.current.texture, data.current.collection)
		end
	end)
